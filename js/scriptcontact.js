...
var failedFiles = [];
    var ul = $('#uploadcontact ul');
	 ...
	 var $image_format = 'jpg|jpeg|jpe|png|gif|zip|psd|PSD|doc|pdf|AI|ai|RAR|msword|vnd.openxmlformats-officedocument.wordprocessingml.document|vnd.adobe.photoshop|x-photoshop|postscript';
    ...
    $('#uploadcontact').fileupload({
        ...
        add: function (e, data) {
             ...
             // $('#contactform').attr('action','uploadcontact.php');
              $('#contactform').attr('action','upload.php?frm=contact');
             ...
             // validate all formats first
                if($all_formats){
                    ...
                    if(data.files[index]['type'].length && !acceptFileTypes.test(data.files[index]['type'])) {
                        uploadErrors.push('Not an accepted file type. Only jpg,jpeg,jpe,png,gif,zip,psd,doc,pdf');
                      ...
                      ttlSize = parseFloat(ttlSize) + parseFloat(fileSize);
                       ...
                       // if((data.files[index]['size'] >= $image_size * mb) || (ttlSize >= $image_size * mb)) {
                        if((data.files[index]['size'] >= $image_size * mb)) {
                            uploadErrors.push('Filesize is too big not more than 20MB');
            ...
            if(data.files.length){
                ...
                ' data-fgColor="#0788a5" data-readOnly="1" data-bgColor="#3e4043" /><p></p><span class="x '+ranValue+'"></span></li>');
      ...
      //  alert(ul);
        ...
        progress: function(e, data){
           ...
           $("#submitButton").addClass('upload-disable');
            ...
            if(progress == 100){
                ...
                $("#submitButton").removeClass('upload-disable');
