...
$subscribe_class = '';
if( !empty($epcl_theme['bg_subscribe']['url']) ){
    $subscribe_class = 'lazy';
}
?>
<!-- start: #footer -->
<footer id="footer" class="grid-container np-mobile <?php echo esc_attr($footer_class); ?>">
	...
	<?php if( epcl_get_option('mailchimp_url') && epcl_get_option('enable_subscribe') == '1' ): ?>
		<div class="subscribe section <?php echo esc_attr($subscribe_class); ?>" <?php if( !empty($epcl_theme['bg_subscribe']['url']) ): ?>style="background-image: none;" data-src="<?php echo esc_url( $epcl_theme['bg_subscribe']['url'] ); ?>"<?php endif; ?>>
	...
	<?php if( epcl_get_option('copyright_text') ): ?>
		<div id="dynamicdate" class="published border-effect"> 
			<p>Copyright © 2014 - <?php echo date('Y'); ?>. All Rights Reserved. <a href="https://www.techieapps.com/">techieapps.com</a></p>
		</div>
	<?php endif; ?>
    <?php if( empty($epcl_theme) || $epcl_theme['enable_back_to_top'] == '1' ): ?>
        <a href="javascript:void(0)" id="back-to-top" class="epcl-button dark"><img src="<?php echo EPCL_THEMEPATH; ?>/assets/images/top-arrow.svg" width="15" alt="<?php esc_attr_e('Back to top', 'reco'); ?>"></a>
    <?php endif; ?>
